...
async function applyNumberVerified(domainUser: DashboardUser['domainUser'], number: string) {
	...
	await db
		...
		.where(and(eq(contacts.channel, 'whatsapp'), eq(contacts.value, number), isNull(contacts.deletedAt)));
	await db.insert(contacts).values({ userId: domainUser.id, channel: 'whatsapp', value: number, verifiedAt: new Date() });
	// `verified.whatsapp` means "at least one WhatsApp number is verified".
	await db.update(users).set({ verified: { ...domainUser.verified, whatsapp: true } }).where(eq(users.id, domainUser.id));
}
...
export const load: PageServerLoad = async (event) => {
	...
	const [existing] = await db
		...
		.from(contacts)
		.where(and(eq(contacts.userId, domainUser.id), eq(contacts.channel, 'whatsapp'), eq(contacts.value, number), isNull(contacts.deletedAt)));
	if (existing?.verifiedAt) redirect(302, withNotice(next, `${number} is already verified.`));
	if (await verifiedByOther(domainUser.id, number)) {
	...
	if (!(await hasPendingOtp('whatsapp', number))) {
		try {
			await sendOtp(domainUser.id, 'whatsapp', number);
	...
	return { next, phone: number, phoneCooldown };
};
...
export const actions: Actions = {
	sendPhoneCode: async (event) => {
		...
		try {
			...
			await sendOtp(domainUser.id, 'whatsapp', normalized);
		} catch (error) {
	...
	verifyCode: async (event) => {
		...
		const result = await verifyOtpWithDestination(domainUser.id, 'whatsapp', code);
		if (!result.ok || !result.destination) return fail(400, { codeError: 'That code is invalid or expired.' });

		await applyNumberVerified(domainUser, result.destination);
