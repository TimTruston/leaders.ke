...
<div>
	...
	<form method="get" class="mt-6 flex items-center gap-2">
		...
		<input
			...
			name="q"
			value={data.q}
			placeholder="Search leader or claimant…"
	...
	{#if data.claims.length > 0}
			...
			<table class="w-full min-w-180 border-collapse text-left">
				<thead>
					<tr class="bg-surface-2">
						...
						{@render sortableTh('subject', 'Leader')}
						{@render sortableTh('claimant', 'Claimant')}
		...
		<Pagination page={data.page} {totalPages} total={data.total} itemLabel="claims" href={(p) => buildHref({ page: p })} />
	{:else}
		<p class="mt-6 text-sm text-muted">{data.q ? `No claims match “${data.q}”.` : 'No profile claims yet.'}</p>
	{/if}
</div>
