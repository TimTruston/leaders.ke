...
// where the dashboard layout's own (already-computed) myCampaigns/pendingClaims
// aren't available — running this query on every public page's SSR load would tax
// every page view for every signed-in visitor instead of just an actual switcher open.
...
export const GET: RequestHandler = async (event) => {
	...
	return json({ myCampaigns, pendingClaims, isAdmin: !!domainUser.adminAt });
};
